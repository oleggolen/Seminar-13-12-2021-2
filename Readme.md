...
Для слияния веток используется команда *git merge*. Принимается она в папке с репозиторием следующим образом: *git merge <название ветки>*. В процессе слияния может произойти ***КОНФЛИКТ*** или слияние может произойти автоматически

## Работа с удаленными репозитариями

### Копирование удаленного репозитария

Команда *git clone* позволяет скопировать удаленный (внешниий) репозиторий на ваш ПК. Пишется она следующим образом: *git clone <ссылка>*, где *<ссылка>* - удаленного репозитория

### Отправка текущего репозитория

Команда *git push* отправляет нашу версию репозитория на внешний репозиторий. Пишется она следующим образом: *git push origin <название ветки>*. Требуется авторизация на внешнем репозитории.

### Скачивание удаленного репозитория

Команда *git pull* позволяет скачать все из удаленного репозитария и автоматически сделать merge с нашей версией. Пишется она следующим образом: *git pull origin <название ветки>*.
